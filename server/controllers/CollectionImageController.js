...
const addImageToCollection = async (req, res) => {
  try {
    ...
    if (result.status)
      return res
        .status(200)
        .json({ message: "ImageSearchView added to collection" });
...
const removeImageFromCollection = async (req, res) => {
  try {
    ...
    const result = await service.removeImageFromCollection(
      collectionId,
      imageId,
    );
    if (result.status)
      return res
        .status(200)
        .json({ message: "ImageSearchView removed from collection" });
