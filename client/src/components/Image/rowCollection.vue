...
const filteredCollections = computed(() => {
	const filtered = allCollections.value.filter(collection => collection.name.toLowerCase().includes(searchText.value.toLowerCase()));
...
<template>
  		<div class="w-full h-full gap-4 cursor-pointer">
				<div
					...
					:key="collection.id"
					class="w-full h-[75px] transition-all flex flex-row items-center rounded-md p-2 hover:bg-lightGreyCustom group">
